...
export function* handlIncreaseItemQuantity({ id }) {
...
export function* handlDecreaseItemQuantity({ id, local }) {
  if (local) {
    return;
  }
  yield put(setItemQuantityFetchStatus(FETCHING));
  ...
  if (response.status !== 200) {
    alert('Receive non 200 status', response);
  }
  yield put(setItemQuantityFetchStatus(FETCHED));
}
...
export function* itemQuantitySaga() {
  yield [
    takeLatest(DECREASE_ITEM_QUANTITY, handlDecreaseItemQuantity),
    takeLatest(INCREASE_ITEM_QUANTITY, handlIncreaseItemQuantity)
  ];
}
